...
module.exports.unloadModule = function ({ moduleList, threadID, messageID, getText }) {
    ...
    const { configPath, mainPath, api } = global.client;
    const logger = require(mainPath + "/pdata/utils/log").loader;
    ...
    switch (args[0]) {
         case "count": {
              let events = client.events.values();
		          let infoEvents = "";
			        api.sendMessage("» Hiện tại đang có " + client.events.size + " lệnh có thể sử dụng!"+ infoEvents, event.threadID, event.messageID);
           break;
		}
        case "load": {
